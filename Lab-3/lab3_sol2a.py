...
# Vlad part
fileList = ['Eliot.txt', 'Tolkien.txt', 'DB.txt']
rows = len(fileList)
fileContent = [""]*rows
...
for i,file in enumerate( fileList ):
    # print(i,file)
    fileContent[i] = preProcess(readFile(file))
...
for i in range(len(fileContent)):
    chunks1, count1 = partition_str(fileContent[i] , wind)
    # print(chunks,"\n\n")
    chunks += chunks1
    rows += count1
...
np.save('dist1WithDB',dist1,allow_pickle = True)  
